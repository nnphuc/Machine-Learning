...
class KMeans(object):
  ...
  def train(self, X, epsilon=1e-12):
    ...
    self.clusters = np.array([X[i] for i in np.random.choice(X.shape[0],self.num_clusters)])
    ...
    while (all(num > epsilon for num in change)):
        ...
        cluster_index = np.array([np.argmin(dists[i]) for i in range(X.shape[0])])
        
        new_clusters = np.zeros_like(self.clusters)
        for j in range(self.num_clusters):
            #print(X[cluster_index==j])
            new_clusters[j] = np.mean(X[cluster_index==j])
        
            change[j] = np.abs(np.sum(self.clusters[j] - new_clusters[j]))
  ...
  def compute_distances_two_loops(self, X):
    ...
    for i in range(num_test):
      for j in range(self.num_clusters):
        ...
        #####################################################################
        dists[i,j] = np.linalg.norm(X[i] - self.cluster[j])
        #####################################################################
        #                       END OF YOUR CODE                            #
        #####################################################################
    return dists
  ...
  def compute_distances_one_loop(self, X):
    ...
    for i in range(num_test):
      ...
      #######################################################################
      dists[i, :] = np.sqrt(np.sum(np.square(X[i,np.newaxis] - self.clusters), axis=1))
      #######################################################################
      #                         END OF YOUR CODE                            #
      #######################################################################
    return dists
  ...
  def compute_distances_no_loops(self, X):
    ...
    #########################################################################
    dists = np.sqrt(np.sum(np.square(X[:,np.newaxis,:] - self.clusters), axis=2))
    #########################################################################
    #                         END OF YOUR CODE                              #
    #########################################################################
    return dists
